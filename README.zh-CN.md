...
- **📍 朗读线 + 永不读串行**：一条高亮「当前读到哪」的参考线，可随手拖到舒服的高度；朗读时当前字会高亮、已读过的文字自动变淡，眼睛永远对准正确的位置。
...
- **语音跟随滚动**：基于浏览器内置语音识别，你念到哪、稿子滚到哪；识别到的是「增量」还是「重说」，都会智能对齐，不会跳回开头。
- **开始 / 停止**，运行中可**暂停 / 继续**。
...
- **滚轮灵敏度可调**：自定义每滚一格文字移动多远，贴合你的习惯，而不是一动就飞出去。
...
### 窗口与显示
- **浮窗模式**：标题栏（或可见的拖拽手柄）可拖拽移动，右下角可拖拽缩放，自由摆放在屏幕任意位置。
- **窗口全屏**：填满主区域，不被设置面板挤占。
